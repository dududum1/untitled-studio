...
class GestureHandler {
    ...
    handleTouchMove(e) {
        if (e.touches.length === 2 && this.state.isPinching) {
            ...
            const scaleFactor = distance / this.state.initialDistance;

            // Dampening for smoothness
            ...
            let newX = focalX - (focalX - this.state.translateX) * scaleRatio;
            let newY = focalY - (focalY - this.state.translateY) * scaleRatio;
            ...
            // Soft Boundaries (Rubber banding - visual only, snap happens on end)
            const bounds = this.getBoundaries();
            // We apply rubber banding during move for feedback
